...
const ZONES = [
    'Madrid Centro','Salamanca','Chamberí','Retiro','Arganzuela',
    'Carabanchel','Vallecas','Hortaleza','Fuencarral','Moncloa',
    'Latina','Tetuán','Ciudad Lineal','Barajas','Vicálvaro',
];
const LANGUAGES = [
    'Español','Inglés','Francés','Alemán','Italiano',
    'Portugués','Árabe','Chino','Rumano','Ruso','Ucraniano','Catalán',
];
const CARE_TYPES = [
...
function Tag({ label, onRemove }: { label:string; onRemove:()=>void }) {
    return (
        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#2e6fba] text-white text-xs font-semibold"
            style={{ fontFamily: dmSans }}>
            {label}
...
// ─── Mobile Top Bar ───────────────────────────────────────────────────────────
// Each step column is exactly 25% wide → circle center sits at (idx*25 + 12.5)%
// which matches the logo's `left` value, so the logo always lands precisely on the circle.
        ...
        <div className="lg:hidden sticky top-0 z-20" style={{ background: '#0e2d5a' }}>
            ...
            <div className="relative z-10 px-4 pt-3 pb-3">
                <div className="relative">
                    ...
                    {/* ── Floating logo — moves to active step circle ── */}
                    <div
                        style={{
                            position: 'absolute',
                            top: 0,
                    ...
                    {/* ── Step row — each step is exactly 25% wide ── */}
                    <div className="relative flex w-full" style={{ paddingTop: '40px' }}>

                        {/* Connector lines: absolutely positioned between circle edges */}
                        {[0, 1, 2].map(i => (
                            <div
                                key={i}
                                ...
                                style={{
                                    ...
                                    top: '16px', // vertically centred with 32 px circles
                                ...
                                >
                                    ...
                                    <div
                                        ...
                                        style={{
                                            ...
                                            boxShadow: active
                                                ? '0 0 0 4px rgba(46,111,186,0.25), 0 2px 12px rgba(46,111,186,0.4)'
                                                : 'none',
                                            position: 'relative',
                                            zIndex: 1,
                                        }}
                                    >
                                        {done
                                            ...
                                            : <span className="text-xs font-bold" style={{
                                                fontFamily: dmSans,
                                                color: active ? '#2e6fba' : 'rgba(113,174,221,0.4)',
                                            }}>{s.num}</span>
                                        }
                                    </div>
                                    ...
                                    <span
                                        ...
                                        style={{
                                            fontFamily: dmSans,
                                            color: active ? '#ffffff' : done ? '#71aedd' : 'rgba(113,174,221,0.35)',
                                        }}
                                    >
                                        {s.label}
                                    </span>
            ...
            <div className="relative z-10 flex flex-col h-full px-7 py-8">
                                    ...
                                    <div className="flex items-center justify-between w-full" style={{ height: '40px' }}>
                                        <div>
                                            <p className="text-sm font-semibold leading-tight transition-all duration-300"
                                                style={{
                                                    fontFamily: dmSans,
                                                    color: active ? '#ffffff' : done ? '#a4c8ee' : 'rgba(113,174,221,0.4)',
                                                }}>
                                                {s.label}
                                            </p>
                                            <p className="text-xs leading-tight mt-0.5 transition-all duration-300"
                                                style={{
                                                    fontFamily: dmSans,
                                                    color: active ? '#71aedd' : done ? 'rgba(164,200,238,0.5)' : 'rgba(113,174,221,0.25)',
                                                }}>
                                                {s.sub}
                                            </p>
                                        </div>
                                        {done && (
                                            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full flex-shrink-0"
                                                style={{
                                                    ...
                                                    color:'#71aedd',
                                                    border:'1px solid rgba(46,111,186,0.3)',
                                                }}>
                                                ✓
                                            </span>
                                        )}
                ...
                <div className="mt-8 rounded-2xl p-4"
                    style={{
                        ...
                        border:'1px solid rgba(164,200,238,0.12)',
                        backdropFilter:'blur(8px)',
                    }}>
                    <div className="flex items-center gap-2 mb-2">
                        <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs"
                            style={{ background:'rgba(46,111,186,0.3)' }}>
                            ⏱
                        </div>
        ...
        <div>
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-3 pt-1">
                    {[
                        { key:'terms', label: <>Acepto los <a href="#" className="font-semibold hover:underline" style={{ color:'#2e6fba' }}>Términos y Condiciones</a> y la <a href="#" className="font-semibold hover:underline" style={{ color:'#2e6fba' }}>Política de Privacidad</a></> },
                        { key:'data_consent', label:'Consiento el tratamiento de mis datos personales para la gestión de la plataforma' },
                    ].map(({ key, label }) => (
                        <label key={key} className="flex items-start gap-2.5 cursor-pointer">
                            <div onClick={() => setData(key, !data[key])}
            ...
            <div className="space-y-4">
                ...
                <div>
                    <div className="flex items-center gap-2 mb-1">
                        <label className="text-sm font-semibold" style={{ fontFamily: dmSans, color:'#0e2d5a' }}>Área Ocupacional</label>
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold"
                            style={{ background:'rgba(46,111,186,0.1)', color:'#2e6fba', border:'1px solid rgba(46,111,186,0.2)', fontFamily: dmSans }}>
                            📍 Madrid
                        </span>
                    </div>
                    <p className="text-xs mb-2.5" style={{ fontFamily: dmSans, color:'#5a96d4' }}>Selecciona los barrios/distritos donde puedes trabajar</p>
                    <div className="flex flex-wrap gap-2">
                        {ZONES.map(z => (
                            <Chip key={z} label={z} active={data.zones.includes(z)}
                                onClick={() => setData('zones', toggle(data.zones, z))} />
                        ))}
                    </div>
                </div>
                <div>
                    <label className="block text-sm font-semibold mb-1" style={{ fontFamily: dmSans, color:'#0e2d5a' }}>Idiomas</label>
                    <p className="text-xs mb-2.5" style={{ fontFamily: dmSans, color:'#5a96d4' }}>Selecciona todos los idiomas en los que puedes comunicarte</p>
                    <div className="flex flex-wrap gap-2 mb-3">
                        {LANGUAGES.map(l => (
                            <Chip key={l} label={l} active={data.languages.includes(l)}
                                onClick={() => setData('languages', toggle(data.languages, l))} />
                        ))}
                    </div>
                    {data.languages.length > 0 && (
                        <div className="flex flex-wrap gap-2">
                            {data.languages.map((l: string) => (
                                <Tag key={l} label={l} onRemove={() => setData('languages', toggle(data.languages, l))} />
                            ))}
                        </div>
                    )}
                </div>
